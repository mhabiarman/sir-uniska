...
$query = mysqli_query($conn, "SELECT * FROM tb_surat_kematian WHERE id_surat_kematian = '$id'");
$row = mysqli_fetch_array($query);

$uploadDir = '../assets/images/berkas';

if(!empty($_FILES['fc_ktp_pelapor']['name'])) {
	unlink($uploadDir."/".$row['fc_ktp_pelapor']);
	...
	$gambarnama_1 = $row['fc_ktp_pelapor'];	
}
...
$query2 = mysqli_query($conn, "UPDATE tb_surat_kematian SET status = '$status', c1 = 0, c2 = 0, c3 = 0, k1 = '', k2 = '', k3 = '', fc_ktp_pelapor = '$gambarnama_1', fc_kartu_keluarga = '$gambarnama_2', fc_surat_rt = '$gambarnama_3', modified = NOW() WHERE id_surat_kematian = '$id'");
if($query2) {
