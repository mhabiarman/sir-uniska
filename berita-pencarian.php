...
if(isset($_GET['keyword'])) {
	...
	$slug_keyword = preg_replace('/[^a-z0-9]+/i', '-', trim(strtolower($keyword)));
	header("location: berita-pencarian?id=".$slug_keyword."");
}
?>
<!DOCTYPE html>
<html lang="en">
	...
	<section class="ftco-section bg-light">
		<div class="container">
			<div class="row d-flex">
				...
				$array = explode('-', $id);
				$acak = implode('|', $array);
				$query = mysqli_query($conn, "SELECT * FROM tb_berita WHERE judul REGEXP'$acak' ORDER BY id_berita ASC");
				while($row = mysqli_fetch_array($query)) {
					?>
					<div class="col-md-4 d-flex ftco-animate">
						<div class="blog-entry justify-content-end">
							<a href="berita/<?php echo $row['id_berita'];?>" class="block-20" style="background-image: url('assets/images/berita/<?php echo $row['gambar'];?>');">
							</a>
							<div class="text p-4 float-right d-block">
								...
								<h3 class="heading mb-0"><a href="berita/<?php echo $row['id_berita'];?>"><?php echo $row['judul'];?></a></h3>
								<p><?php echo $row['keterangan'];?></p>
								<p><a href="berita/<?php echo $row['id_berita'];?>" class="btn btn-primary">Selengkapnya</a></p>
							</div>
						</div>
					</div>
				<?php } ?>
